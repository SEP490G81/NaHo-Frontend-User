...
export async function getHistoryList(): Promise<QuestionHistoryEntry[]> {
    ...
    try {
        ...
        console.warn(
            "getHistoryList API failed, falling back to mock data:",
            error,
        );
        return getFallbackHistory();
    }
...
export async function getHistoryById(
    ...
    if (!hasApiConfigured()) {
        const entry = getFallbackHistory().find(
    ...
    try {
        return await apiFetch<QuestionHistoryEntry>(
            `/api/v1/history/${historyId}`,
        );
    } catch (error) {
        console.warn(
            `getHistoryById API failed for historyId ${historyId}, falling back to mock data:`,
            error,
        );
        const entry = getFallbackHistory().find(
            (h) => h.historyId === historyId,
...
export async function submitPractice(
    ...
    try {
        ...
        console.warn(
            "submitPractice API failed, falling back to local simulation:",
            error,
        );
        const newId = `h-${Date.now()}`;
        const score = Math.round((6.0 + Math.random() * 3.5) * 10) / 10;
