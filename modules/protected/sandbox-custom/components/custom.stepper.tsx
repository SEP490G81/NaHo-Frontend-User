...
export function CustomStepper({ step }: CustomStepperProps) {
  ...
  return (
    <ol className="flex w-full items-center justify-between gap-2 rounded-xl border border-bdc-primary bg-bgc-app p-3 md:p-4 shadow-sm">
      {steps.map((s, i) => {
        const done = s.id < step;
        const active = s.id === step;
        return (
          <li key={s.id} className="flex flex-1 items-center gap-2 md:gap-3">
            <div
              className={cn(
                ...
                !done && !active && "border-bdc-muted text-text-muted",
              )}
            >
            ...
            {i < steps.length - 1 && (
              <span
                className={cn(
                  ...
                  done ? "bg-bgc-highlight" : "bg-bdc-primary",
                )}
              />
            )}
          </li>
