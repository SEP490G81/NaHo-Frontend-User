...
import { SandboxCustomProvider, useSandboxCustom } from "../provider/sandbox-custom.context";
  ...
  if (!current || current.id !== id) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-12 animate-fade-in">
        <div className="rounded-2xl border border-bdc-primary bg-bgc-app p-8 text-center shadow-sm">
          <h1 className="text-xl font-bold text-text-contrast">{t("noQuestionTitle")}</h1>
          <p className="mt-2 text-sm text-text-muted">
            {t("noQuestionSubtitle")}
          </p>
          <Link
            href="/custom-question"
            className="mt-5 inline-flex h-9 items-center justify-center rounded-md bg-bgc-highlight px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-bgc-highlight/90 shadow-sm"
          >
            {t("noQuestionBtn")}
          </Link>
        </div>
      ...
      <div className="mx-auto max-w-5xl space-y-5">
        <header className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="furigana-custom" className="cursor-pointer text-sm text-text-contrast font-medium">
              {t("showFurigana")}
            </label>
            <Switch
              id="furigana-custom"
