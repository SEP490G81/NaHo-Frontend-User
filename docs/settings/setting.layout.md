...
- Tôi muốn bạn sửa lại `app/[locale]/(protected)/settings/layout.tsx` sao cho toàn bộ trang setting sẽ nằm gọn trong màn
  hình (không scroll), các header và bottom 20px.
- Chỉ có phần body của setting mới được scroll (tức là khi scroll thì sidebar,... sẽ cố định)
