...
- Tôi muốn bạn sửa lại cho tôi `types/responses/speaking.response.ts` như sau:

+ các chỗ sessionId mà có kiểu string thì bạn phải đổi thành sessionCode
+ Sửa lại các file mà có sử dụng nó
+ Tôi mới đổi API backend từ /speaking/session/{personaId} thành /speaking/session/persona/{personaId}
