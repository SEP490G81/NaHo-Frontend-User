...
- Tôi muốn bạn sửa lại cho tôi file `modules/protected/dialogue-setup/components/companion-list.tsx` như sau:

+ Tôi chỉ muốn tối đa 3 `CompanionCard` trên 1 row.
+ Nếu như màn hình nhỏ thì phải có responsive để giảm số card trên 1 row xuống thành 2, hoặc 1.
+ Chỉ hiển thị tối đa 3 row, còn đâu sẽ phải có thanh scrollbar để cuộn xem thêm các personas.
