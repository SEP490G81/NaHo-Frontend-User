...
- Với cái chức năng ghim 1 session với AI vào thanh sidebar `layouts/sidebar` tôi cần cải thiện như sau:

+ Khi tôi start 1 session `modules/protected/dialogue-setup/components/summary-panel.tsx` thì phải ghim vào luôn (tức là
  gọi API lấy các in progress)
+ Trong trang `app/[locale]/(protected)/live-chatroom/page.tsx` cần có nút trở lại trang trước
+ Xóa cái `modules/protected/dialogue-setup/components/resume-banner.tsx`
+ Khi 1 bài được completed thì cũng xóa nó khỏi ghim (nhưng mà không được gọi API xóa)
