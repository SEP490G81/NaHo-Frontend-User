...
- Tôi muốn bạn sửa lại page `app/[locale]/(protected)/live-chatroom/page.tsx` với logic như sau:

1. Khi người dùng set up các cái như persona, formalityLevel, marugotoLevel ở
   `modules/protected/dialogue-setup/components` rồi ấn nút start trong
   `modules/protected/dialogue-setup/components/summary-panel.tsx` thì phải gọi API `startConversation` để nhận
   sessionCode
2. Sau khi nhận được sessionCode, sẽ điều hướng người dùng tới trang /live-chatroom/{sessionCode}, rồi nextjs phía
   server sẽ lấy ra code đó và gọi API `getInProgressSessionDetails`, nếu `messages` rỗng thì sẽ có nút sẵn sàng, khi ấn
   vào sẽ gọi API `initFirstGreeting`
3. Tái sử dụng component `modules/protected/live-chatroom/components/live-chatroom.tsx` nếu như người dùng gửi message
   thì gọi hàm `sendMessage` nếu gửi audio thì phải convert audio về dạng wav sau đó gọi hàm `sendAudioMessage`
