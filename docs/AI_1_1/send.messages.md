...
- Tôi muốn làm chức năng send message gồm 2 phần như sau:

1. Send text message:

- Gọi API backend: `sendMessage`

+ URL: `http://localhost:8386/api/v1/speaking/session/message`
+ Method: `POST`
+ Request body:
...
- Gọi API backend: `sendAudioMessage`

+ URL: `http://localhost:8386/api/v1/speaking/session/audio/fceea00c-9a93-4b41-8e14-e7e96854215f`
+ Method: `POST`
+ Request body (form-data): file = `audio file`
+ Response:
...
- Sau khi gọi message thì phải nối luôn vào các đoạn chat đang có
- Có hiệu ứng loading khi gửi message
- Trong trường hợp trả ra lỗi: `LLM_A006` thì tức là đoạn chat đã đạt tới giới hạn message gửi trong 1 phiên nên không
  thể gửi nữa.
- Ngoài ra ở chỗ `cài đặt hội thoại` trong mục chat room thì cũng cần hiệu ứng loading rồi mới hiển thị persona
  info,trinhf độ, phong cách...
