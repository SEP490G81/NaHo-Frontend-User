...
export function HeaderDecoration() {
    return (
        <>
            ...
            <div className="absolute top-0 left-0 h-full w-48 pointer-events-none select-none opacity-50 dark:opacity-25 z-0">
                    ...
                    <g className="text-bgc-highlight" fill="currentColor">
                        <g transform="translate(60, 15) rotate(15)">
                            {Array.from({ length: 5 }).map((_, p) => (
                                <ellipse key={p} cx={0} cy={-4} rx={2} ry={4.5} transform={`rotate(${p * 72})`} opacity="0.9" />
                            ))}
                            <circle r="1" fill="var(--color-bgc-page)" />
                        </g>
                        <g transform="translate(110, 10) rotate(45)">
                            {Array.from({ length: 5 }).map((_, p) => (
                                <ellipse key={p} cx={0} cy={-3.5} rx={1.8} ry={4} transform={`rotate(${p * 72})`} opacity="0.9" />
                            ))}
                            <circle r="0.8" fill="var(--color-bgc-page)" />
                        </g>
                        <g transform="translate(150, 20) rotate(90)">
                            {Array.from({ length: 5 }).map((_, p) => (
                                <ellipse key={p} cx={0} cy={-3} rx={1.5} ry={3.5} transform={`rotate(${p * 72})`} opacity="0.9" />
                            ))}
            ...
            <div className="absolute top-0 right-0 h-full w-48 pointer-events-none select-none opacity-50 dark:opacity-25 z-0">
                <svg viewBox="0 0 200 60" className="w-full h-full fill-none">
                    <path
